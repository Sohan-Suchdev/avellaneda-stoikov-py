...
def _max_drawdown(wealth_history: list[float]) -> float:
    max_drawdown = 0.0
    running_peak = None
    ...
    for wealth in wealth_history:
        ...
        drawdown = (running_peak - wealth) / running_peak
...
def calculate_metrics(
    ...
    if pd.isna(std_dev) or std_dev == 0:
        ...
        sharpe = (returns.mean() / std_dev) * np.sqrt(sharpe_annualization_factor)
    ...
    if not pd.isna(downside_deviation) and downside_deviation != 0:
        sortino = (
            returns.mean()
            / downside_deviation
            * np.sqrt(sharpe_annualization_factor)
        )
