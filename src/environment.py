...
class MarketEnvironment:
    ...
    def _fill_probability(self, quote):
        if quote is None:
            return 0.0

        delta = abs(self.mid_price - quote)
        intensity = self.config.A * np.exp(-self.config.k * delta)
        return intensity * self.config.dt

    def execute_orders(self, bid, ask):
        p_buy = self._fill_probability(bid)
        p_sell = self._fill_probability(ask)
        ...
        # Keep Poisson step probabilities valid before Bernoulli draws.
