...
class MarketMaker:
    ...
    def _apply_quote_controls(self, mid_price, bid, ask):
...
class InventorySkewedMarketMaker(NaiveMarketMaker):
...
class VolatilityScaledMarketMaker(NaiveMarketMaker):
    def __init__(
        self,
        spread,
...
class AvellanedaStoikov(MarketMaker):
    ...
    def optimal_total_spread(self, t, sigma=None):
        sigma = self.sigma if sigma is None else sigma
        time_left = max(self.T - t, 0)
        term1 = self.gamma * (sigma**2) * time_left
...
class EnhancedAvellanedaStoikov(AvellanedaStoikov):
    def get_quotes(self, current_time, mid_price, realized_sigma=None):
